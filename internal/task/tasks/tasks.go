/*
 *  Copyright (c) 2021 NetEase Inc.
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 */

/*
 * Project: CurveAdm
 * Created Date: 2021-10-15
 * Author: Jingli Chen (Wine93)
 */

package tasks

import (
	"sync"

	"github.com/fatih/color"
	"github.com/opencurve/curveadm/internal/task/task"
	"github.com/vbauerster/mpb/v7"
	"github.com/vbauerster/mpb/v7/decor"
)

type (
	monitor struct {
		err    error
		result map[int]error
		mutex  sync.Mutex
	}

	ExecOption struct {
		Concurrency   uint
		SilentMainBar bool
		SilentSubBar  bool
		SkipError     bool
	}

	Tasks struct {
		tasks []*task.Task
	}
)

func newMonitor() *monitor {
	return &monitor{
		err:    nil,
		result: map[int]error{},
		mutex:  sync.Mutex{},
	}
}

func (m *monitor) error() error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	return m.err
}

func (m *monitor) set(id int, err error) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	m.result[id] = err
	if m.err == nil {
		m.err = err
	}
}
func (m *monitor) get(id int) error {
	m.mutex.Lock()
	defer m.mutex.Unlock()
	return m.result[id]
}

func newShowStatusCb(monitor *monitor) func(static decor.Statistics) string {
	return func(static decor.Statistics) string {
		if static.Completed {
			if monitor.get(static.ID) == nil {
				return color.GreenString("[OK]")
			} else {
				return color.RedString("[ERROR]")
			}
		}
		return ""
	}
}

func NewTasks() *Tasks {
	return &Tasks{
		tasks: []*task.Task{},
	}
}

func (ts *Tasks) AddTask(t *task.Task) {
	ts.tasks = append(ts.tasks, t)
}

func (ts *Tasks) GetTask() []*task.Task {
	return ts.tasks
}

/*
 * Pull image: [SUCCESS]
 *   + host=10.0.0.1 image=opencurvedocker/curvefs [OK]
 *   + host=10.0.0.2 image=opencurvedocker/curvefs [OK]
 *   + host=10.0.0.3 image=opencurvedocker/curvefs [ERROR]
 *   + host=10.0.0.1 image=opencurvedocker/curvefs [OK]
 *   + host=10.0.0.2 image=opencurvedocker/curvefs [OK]
 *   + host=10.0.0.3 image=opencurvedocker/curvefs [OK]
 */
func (ts *Tasks) Execute(option ExecOption) error {
	tasks := ts.tasks
	if len(tasks) == 0 {
		return nil
	}

	concurrency := option.Concurrency
	if concurrency == 0 {
		concurrency = 3
	}

	var mainbar, bar *mpb.Bar
	wg := sync.WaitGroup{}
	workers := make(chan struct{}, concurrency)
	p := mpb.New(mpb.WithWaitGroup(&wg))
	monitor := newMonitor()
	showStatus := newShowStatusCb(monitor)

	if !option.SilentMainBar {
		mainbar = p.Add(1, nil,
			mpb.PrependDecorators(
				decor.Name(tasks[0].Name()+": "),
				decor.OnComplete(decor.Spinner([]string{}), ""),
				decor.Any(showStatus),
			),
		)
	}

	for _, t := range tasks {
		if monitor.error() != nil && option.SkipError == false {
			break
		}

		wg.Add(1)
		workers <- struct{}{}
		if !option.SilentSubBar {
			bar = p.Add(1, nil,
				mpb.PrependDecorators(
					decor.Name("  + "),
					decor.Name(t.Subname()+" "),
					decor.OnComplete(decor.Spinner([]string{}), ""),
					decor.Any(showStatus),
				),
			)
		}

		// worker
		go func(t *task.Task, bar *mpb.Bar) {
			defer func() {
				if bar != nil {
					bar.IncrBy(1)
				}
				<-workers
				wg.Done()
			}()

			err := t.Execute()

			id := 0
			if bar != nil {
				id = bar.ID()
			}

			monitor.set(id, err)
		}(t, bar)
	}

	wg.Wait()
	if mainbar != nil {
		mainbar.IncrBy(1)
		monitor.set(mainbar.ID(), monitor.error())
	}
	p.Wait()
	return monitor.error()
}
