...
func (s *step2CreateCurveFSTopology) Execute(ctx *context.Context) error {
...
func (s *step2CreateCurveFSTopology) Rollback(ctx *context.Context) {
}

func (s *step2WaitMdsElectionDone) Execute(ctx *context.Context) error {
	config := ctx.Config()
...
func (s *step2WaitMdsElectionDone) Rollback(ctx *context.Context) {
}

// TODO(@Wine93): refactor build topology
func (s *step2BuildTopology) Execute(ctx *context.Context) error {
...
func (s *step2BuildTopology) Rollback(ctx *context.Context) {
}

func NewCreateTopologyTask(curveadm *cli.CurveAdm, dc *configure.DeployConfig) (*context.Task, error) {
	...
	t := context.NewTask("Create Topology", subname, dc)

	// add step
	path4ToolsBinary := fmt.Sprintf("")
	path4WaitScript := fmt.Sprintf("")
	path4TopoFile := fmt.Sprintf("%s/tools/conf/topology.json")
	t.AddStep(&step.ReadFile{
		ContainerId:      containerId,
		ContainerSrcPath: fmt.Sprintf(""),
	})
	...
	t.AddStep(&step.InstallFile{
		Content:           topology,
		ContainerId:       containerId,
		ContainerDestPath: fmt.Sprintf(""),
	})
	t.AddStep(&step.InstallFile{
		Content:           scripts.Get("wait"),
		ContainerId:       containerId,
		ContainerDestPath: path4WaitScript,
	})
	...
	t.AddStep(&step.ContainerExec{
		ContainerId: containerId,
		Command:     fmt.Sprintf("%s build-topology --confPath=%s"),
	})
